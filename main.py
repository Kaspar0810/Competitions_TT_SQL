...
# class PbWidget(QtWidgets.QProgressBar):
...
def page():
    ...
    if tb == 0:
        ...
        # st = System.select().where(System.title_id == title_id(gamer))
        st_count = len(sf)
        # s = System.select().order_by(System.id).where(System.title_id == ).get()  # находит system id последнего
        # соревнования
        # last_id = sf.id
        if st_count != 1:
