...
def page():
    ...
    if tb == 0:
        ...
        if flag is False:
            ...
            if state is True:  # отмечает чекбокс в зависимости от значения в db -system-
                my_win.checkBox_4.setChecked(True)
            else:
                my_win.checkBox_4.setChecked(False)
            game_in_visible(state, match)  # скрывает или показывает поля ввода счета
            my_win.tableWidget.show()
...
def player_in_table():
    ...
    comp_system.table_made(pv)  # создание таблиц групп со спортсменами согласно жеребъевки
...
def select_player_in_game():
    ...
    if tab == 1:
        ...
        game_in_visible(state=False, flag=fin.score_flag, final="Предварительный")
    elif tab == 4:
        pass
...
def total_game_table(kpt, fin, pv, cur_index):
    ...
    if kpt != 0:  # подсчет кол-во игр из выбора кол-ва игроков вышедших из группы и системы финала
        ...
        if total_final >= total_player:  # подсчитывает все ли игроки распределены по финалам
            result = msgBox.question(my_win, "", "Все игроки распределены по финалам.\n"
...
my_win.radioButton_match_3.toggled.connect(game_in_visible)
